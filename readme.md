### day08(추상메서드, 추상클래스, 인터페이스)
 ...
 - `인터페이스` : class간 의존하고 있는 메소드를 <미완성 됫을지라도, 변경될지라도> 호출하는 구조를 짜게 해주는 Interface
   - A -> B의 methodB()를 의존하고 있다면?
     - B가 언제구현될지, 완성될지 모르므로, `미완성된 체 구현해!`의 interface를 씌운다.
     - A는 `강제로 methodB를 구현한 B`클래스의 객체 -> 메소드를 호출할 수 있게 된다.
     - B의 미완성이라도 구현된 메소드는 내용이 바뀔 수 있다.
