...
- 요약2) 오버라이딩시 switch변수 활용등으로 메소드 재정의, 수정 중에 , 부모꺼 그대로 사용하는 분기 일 떄, `super.메소드()` 사용.
...
5. my) **자식들 -> 새로운 기능(method) 추가 뿐만 아니라 상속후 `기능(method) 수정 or재정의(Override)`한 것도 자식class다.**
   ...
   2. **인스턴스 변수(객체단위로) 스위치변수 -> Override(공통(부모) 기능(메소드) 수정)시 활용할 수 있다.**
...
6. 다형성: 한 객체가 다양한 형태로(하위class지만, 상위class 참조변수에 담기면서 공통기능(method)만 호출될 때)
   1. **하위카테고리라도 공통기능만을 다룬다면, 상위 카테고리로서 다루어질 수 있다.** 
      1. -> 다형성으로서 부모class 참조변수에 담긴다.
   2. **대신 개별 추가/수정된 기능은 포기해야한다(에러남)**
      1. -> 부모class(상위카테고리)참조변수에 담겼다면, 하위 기능(method)호출이 안된다.
### day05(클래스, 생성자)
