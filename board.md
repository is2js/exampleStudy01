...
## 기능 목록
- add(추가) ->list(출력) -> update(수정) -> delete(삭제) -> search(검색) -> add2(데이터 추가, 날짜Util) -> read(상세보기+조회수변수) -> signup(회원Class의 객체를 arraylist에 add)  

## 기능 요구 사항
- 요구사항 - 회원가입
- 회원기능 추가
   - 회원 가입 기능 구현. 명령어 : signup
      - 회원 정보를 입력 받아 회원 정보를 저장해주세요.
      - 회원은 아이디 비밀번호 닉네임으로 구성됩니다.
