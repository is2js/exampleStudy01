...
20. 파일 입출력
    1. 날짜, 랜덤숫자처럼 파일 입출력도 Util패키지에 새로운 클래스 `FileManager`로 만든다.
       1. 1객체당 1개 파일로 저장하는게 좋다.
          1. 각 인스턴스변수를 받아올 수 잇기 getter 또는 public 인변으로 변경
       2. write()시 줄바꿈은 안된다. 줄바꿈은 없어도 되니, 컴퓨터가 이해하도록 `맨끝파일 제외` write( + ",") 맨 끝마다 구분자를 + 넣어준다.
       3. write()시 변수 앞에 "변수명:"의 설명도 넣어주자. 줄바꿈은 사람용~~ 읽어올건 컴퓨터라 뒤에 구분자 + `앞에 변수설명`만 필요하다
       4. Test는 클래스를 만들어서 하되, 개발기능 클래스 객체 생성 후 -> 준비물들 생성은 기존코드에서 `new 객체(` 응으로 검색해서 만들어 쓴다.
    2. 본문 적용
       1. 파일매니져 객체 생성후, 기존 `new 객체 생성 -> list.add`하는 부분 (makeTestData())에 가서, 30개 초기 데이터를 모두 30개 txt로 저장하도록 변경함.
       2. loadFile에서 null직전 까지 while문에서 검사는  
          1. **결과값만 업데이트 변수** -> 끝에서 업데이트 후 while문에 바로 올라가서 검사 가능 했지만
          2. **누적 업데이트 변수로 바뀜** -> 당시의 값을 대변하지 못하므로, while문에 올라가봤자 검사를 못한다.
             1. while문에서는 직전에 나온 값(누적X)가지고 검사를 해야한다.
             2. 누적 업데이트 직전에 ->현재 업데이트값 변수 생성 -> if 검사 break; -> while문 검사는 철회하기
       3. load파일시, 파일경로 ->  `id`로 파라미터를 축약시킨다.
          1. **기존에도 readArticles()로 게시물을 `id로만` list에서 검색해서 가져왔엇음**
    3. txt를 불러와 어디서 사용할지 고민해야하는데
       1. 일단 저장한 `설명: + 객체변수 + 구분자(,)` String을 복사해서가져와  다시 쪼개는 것부터 연습해야한다.
          1. 새로운 Test클래스(`SplitTest`)를 만들어서 쪼갠다.
       2. 기존: 생성시 파라미터 다 집어넣어 완성
          - 새로운: `빈 객체`를 생성할 수 있게, **`노 파라미터 생성자`를 정의**하면서 빈 객체 생성
            - **이후 `빈 객체`에다가 일일이 `빈객체.변수 = 값 하나씩 할당해줘서 완성`하기**
              - cf) Car최소단위객체는 파라미터로 name만 받는 생성자가 있었다.
                  - `일부 변수만` position은 `생성자에 파라미터 없다.` -> `나중에 밖에서 채워줘야한다.` or `항상 정해진 값 가지고 생성됨` 
                    - 나중에 밖에서 채우는게 아니라 기본적으로 default값을 가지고 태어나는 놈이라면?
...
- 파일입출력
  - Test에서 기본 코드 짜보기
  - Class로 옮겨서 저장 메서드 만들기
  - 저장메서드 메서드를 본문용으로 수정후 테스트하기
  - 로드메서드 만들기 -> 파일경로 대신 id로 파일 불러오도록 파라미터  수정하기
  - 로드메서드 사용전, 저장한 txt를 불러와 -> 구분자로 쪼개는 등 split -> 객체로 바꾸는 연습하기
  - 다른 방식으로 객체를 `완성`하기
