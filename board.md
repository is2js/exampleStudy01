...
8. 게시물 상세보기(read)
   2. 조회수 변수 -> 상세보기 구현될 때, `먼저++; 해서 올라간 hit를 보여`준다. **인변 객체 생성시 생성자에서 hit는 0으로 초기화함**
...
## 기능 목록
- add(추가) ->list(출력) -> update(수정) -> delete(삭제) -> search(검색) -> add2(데이터 추가, 날짜Util) -> read(상세보기+조회수) -> 

## 기능 요구 사항
- 요구사항 - 게시물 상세보기

- 게시물 목록에서 내용을 제외했으므로 게시물의 내용을 볼 수 있는 상세보기 기능이 필요합니다.

1. 게시물 상세보기 기능 구현. 명령어 : read

2. 원하는 게시물을 선택해 상세 내용을 볼 수 있야합니다.

3. 없는 게시물일 경우 '게시물이 존재하지 않습니다' 출력
