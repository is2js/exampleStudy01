...
16. Member -> 우수회원개념 도입 -> 우수회원Class + 일반회원Class도 추가한 `하카Class`로 세분화해서 `instanceof`로 분기 나눠 따로 관리하기
    ...
    7. my) 하위카테고리Class별 관리, 처리를 위한
       ...
       2. 처리의 2가지 방법
          ...
          2. [공통기능으로 넣고 다형성-상위카테고리 객체에 담아 일괄처리하되, 오버라이딩을 통한 하위카테고리Class별 내부구현부 수정]
## 기능 목록
...
- member를 하카Class로 나눠서 일반회원 + 우수회원 나눠서 관리하기(다형성 -> 꺼낼땐 instanceof +  형변환후 새변수에 담아 분기안에서만 사용하고 끝냄) -> 회원별 인사 오버라이딩 적용 
- 

## 기능 요구 사항
 - 요구사항 - 일반회원과 우수회원 구별해서 관리
   일반회원과 우수회원을 구별해서 관리해주세요.
   일반회원은 아이디, 비밀번호, 이름
   우수회원은 아이디, 비밀번호, 이름, 포인트
   일반회원 로그인과 우수회원 로그인 구별해서 환영인사
   일반회원의 경우 : 안녕하세요 일반회원 ~님 반갑습니다
   우수회원의 경우 : 안녕하세요 우수회원 ~님 사랑합니다. 회원님의 남은 포인트는 현재 ~입니다.
